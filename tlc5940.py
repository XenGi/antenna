...
class TLC5940 (object):
    def __init__(self, numberof_tlc5940=1, spibus=0, spidevice=0, gsclkpin=4, blankpin=3, vprgpin=5):
        ...
        self.DCLevels = [0 for _ in range(self.numberof_leds)]  # internal DC register
    ...
    def write_all_pwm(self, value=0):
        """
    ...
    def write_all_dc(self, value = 0):
        ...
        self.write_dc([value for _ in range(self.numberof_leds)])

    def write_dc(self, input=[]):
        """
        ...
        dcdata_packed = []
        for i in range(0,len(register),4):
            dcdata_packed += self.pack_4to3(register[i:i+4])
    ...
    def blinkwritealldc(self, times):
        for y in range(times):
            self.write_all_dc(16)  # 16, easy on the eyes...
            sleep(0.02)
            self.write_all_dc(0)
            sleep(0.05)
